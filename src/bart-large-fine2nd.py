...
from transformers import  AdamW, BartConfig, BartTokenizer, BartForConditionalGeneration
...
data = load_data("./ground_truth.json")
...
print(f"number of training smaples = {len(train_data)}")
...
# Check if GPU is available, and if so, move the model to the GPUs
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

# Load pre-trained model and tokenizer onto the 4 GPUs
...
if isinstance(model, nn.DataParallel):
    model.module.save_pretrained('./bulletlawyer32')
else:
    model.save_pretrained('./bulletlawyer32')
...
for batch in test_loader:
    ...
    generated_batch_texts = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True) for g in outputs]
